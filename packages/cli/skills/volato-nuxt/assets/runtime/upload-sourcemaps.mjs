...
const paths = [...mapsUnder(publicRoot), ...mapsUnder(serverRoot)].sort();
...
try {
  for (const path of paths) {
    ...
    const inPublic = relative(publicRoot, path).split(/[\\/]/)[0] !== "..";
    const displayPath = (inPublic ? relative(publicRoot, path) : relative(outputRoot, path))
      .replaceAll("\\", "/")
      .replace(/\.map$/, "");
    const filenameHash = inPublic
      ? browserHash(path)
      : stablePathHash(displayPath);
    if (!filenameHash) {
      throw new Error(`[Volato] Nuxt emitted an unaddressable browser sourcemap: ${path}.`);
    }
    ...
    if (!response.ok) {
      throw new Error(`[Volato] Nuxt sourcemap upload failed with HTTP ${response.status}.`);
    }
    uploaded += 1;
  }
} finally {
...
if (paths.length > 0 && dirty) {
  console.warn(
    "[Volato] Nuxt sourcemaps were removed but not uploaded because the release was inferred from Git and the worktree has uncommitted changes. Commit the build inputs or set VOLATO_RELEASE explicitly for this build.",
  );
} else if (paths.length > 0 && (!dsn || !token || !identity.release)) {
  console.warn(
    "[Volato] Nuxt sourcemaps were removed but not uploaded. Set VOLATO_DSN, VOLATO_INGEST_TOKEN and VOLATO_RELEASE in CI.",
  );
} else if (uploaded !== paths.length) {
  throw new Error(`[Volato] Nuxt uploaded ${uploaded}/${paths.length} sourcemaps.`);
} else if (paths.length > 0) {
  console.log(
    `[Volato] Uploaded ${paths.length} privacy-cleaned Nuxt sourcemap(s) for ${identity.release}.`,
  );
}
