...
describe("bundle hygiene (dist/server.js)", () => {
  const __dirname = dirname(fileURLToPath(import.meta.url));
  const distPath = resolve(__dirname, "../../dist/server.js");

  it("exists (run `pnpm build` before `pnpm test`)", () => {
  ...
  it("exports captureException", () => {
    if (!existsSync(distPath)) {
      throw new Error(
        `Missing ${distPath}. Run \`pnpm build\` before \`pnpm test\`.`,
      );
    }
    const bundle = readFileSync(distPath, "utf8");
    expect(bundle).toMatch(/captureException/);
  });

  it("exports wrapAction, wrapRoute, and reportActionError", () => {
    if (!existsSync(distPath)) {
      throw new Error(
        `Missing ${distPath}. Run \`pnpm build\` before \`pnpm test\`.`,
      );
    ...
    const bundle = readFileSync(distPath, "utf8");
    expect(bundle).toMatch(/wrapAction/);
    expect(bundle).toMatch(/wrapRoute/);
    expect(bundle).toMatch(/reportActionError/);
  });

  it("does not import node:crypto (SDK must stay Edge-safe)", () => {
    if (!existsSync(distPath)) {
      throw new Error(
        `Missing ${distPath}. Run \`pnpm build\` before \`pnpm test\`.`,
      );
